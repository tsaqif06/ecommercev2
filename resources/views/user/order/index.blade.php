                ...
                <div class="col-md-8">
                    <div class="card mb-4" style="border: 0;">
                        <div class="card-body" style="text-align: center">
                            <h5 class="card-title text-center">{{ __('main.get_vip') }}</h6>
                                <h6 class="card-text">★ {{ __('main.be_the_first') }}</h6>
                                <h6 class="card-text">★ {{ __('main.never_lose') }}</h6>
                                <div class="d-flex justify-content-center mt-4">
                                    <button data-toggle="modal" data-target="#registerModal"
                                        class="btn btn-outline-custom mr-2 rounded"
                                        style="width: 100%">{{ __('main.signup') }}</button>
                                    <button data-toggle="modal" data-target="#loginModal"
                                        class="btn btn-custom ml-2 rounded"
                                        style="width: 100%">{{ __('main.login') }}</button>
                                </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content container-fluid rounded" style="max-width: 500px; max-height: 430px;">
                    <div class="modal-body">
                        ...
                        <h5 class="modal-title mb-3" id="registerModalLabel">{{ __('main.register') }}</h5>
                        <p style="font-weight: bold;">{{ __('main.create_an_acc') }}</p>
                        ...
                        <form class="mt-3" method="post" action="{{ route('register.submit') }}">
                            ...
                            <div class="form-floating-custom mb-3 position-relative">
                                ...
                                <label for="registerFullName">{{ __('main.your_fullname') }}*</label>
                            </div>
                            <div class="form-floating-custom mb-3 position-relative">
                                <input type="text" class="form-control" placeholder=" " id="registerEmail" name="email"
                                    value="{{ old('email') }}" required>
                                <label for="registerEmailPhone">{{ __('main.your_email') }}</label>
                                <i class="fas fa-user"></i>
                            </div>
                            <div class="form-floating-custom mb-3 position-relative">
                                <input type="password" class="form-control" placeholder=" " id="registerPassword"
                                    name="password" value="{{ old('password') }}" required>
                                <label for="registerPassword">{{ __('main.password') }}</label>
                                <i class="fas fa-lock"></i>
                            </div>
                            <button type="submit" class="btn btn-primary w-100"
                                style="border-radius: 10px">{{ __('main.create_new_acc') }}</button>
                        </form>

                        <p class="text-center mt-3">
                            {{ __('main.already_have') }}
                            <a href="#" class="switch-to-login">{{ __('main.login_here') }}</a>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content modal container-fluid rounded" style="max-width: 500px; max-height: 300px;">
                    <div class="modal-body">
                        <button type="button" class="btn-close float-end" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                        <h5 class="modal-title mb-3" id="loginModalLabel">{{ __('main.login') }}</h5>
                        ...
                        <form class="mt-3" method="post" action="{{ route('login.submit') }}">
                            ...
                            <div class="form-floating-custom mb-3 position-relative">
                                ...
                                @enderror
                                <label for="loginEmailPhone">{{ __('main.your_email') }}</label>
                                <i class="fas fa-user"></i>
                            </div>
                            <div class="form-floating-custom mb-3 position-relative">
                                ...
                                @error('password')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                <label for="loginPassword">{{ __('main.password') }}</label>
                                <i class="fas fa-lock"></i>
                            </div>
                            <button type="submit" class="btn btn-primary w-100"
                                style="border-radius: 10px">{{ __('main.login') }}</button>
                        </form>

                        <p class="text-center mt-3">
                            {{ __('main.dont_have') }}
                            <a href="#" class="switch-to-register">{{ __('main.signup_here') }}</a>
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row g-0">
                            ...
                            <div class="col-md-8">
                                <div class="card-body p-4">
                                    <h4 class="card-title mb-4">{{ __('main.update_profile') }}</h4>
                                    <form id="profileForm" method="POST"
                                        ...
                                        <div class="mb-4">
                                            <label for="inputPhoto" class="form-label">{{ __('main.update_pp') }}</label>
                                        ...
                                        <button type="submit"
                                            class="btn btn-custom btn-update w-100">{{ __('main.update_profile') }}</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <form id="order-filter-form" method="GET" action="{{ route('user.order.index') }}"
                    ...
                    <select name="status" class="form-control mr-2" id="status-filterorder">
                        ...
                        <option value="unpaid" {{ request('status') == 'unpaid' ? 'selected' : '' }}>
                            {{ __('main.unpaid') }}</option>
                        <option value="paid" {{ request('status') == 'paid' ? 'selected' : '' }}>{{ __('main.paid') }}
                        </option>
